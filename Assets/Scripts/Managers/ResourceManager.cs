...
public class ResourceManager : MonoSingleton<ResourceManager>
{
    ...
    private void OnDownloadErrorFunction(DownloadErrorData downloadErrorData)
    {
        Debug.Log(
            string.Format(
                "下载出错：packageName: {0} 文件名：{1}，错误信息：{2}",
                downloadErrorData.PackageName,
                downloadErrorData.FileName,
    ...
    private void OnDownloadUpdateFunction(DownloadUpdateData downloadUpdate)
    {
        Debug.Log(
            string.Format(
                "所属PackageName ：{0} 文件总数：{1}，已下载文件数：{2}，下载总大小：{3}，已下载大小{4}",
                downloadUpdate.PackageName,
                downloadUpdate.TotalDownloadCount,
                downloadUpdate.CurrentDownloadCount,
                downloadUpdate.TotalDownloadBytes,
                downloadUpdate.CurrentDownloadBytes
            )
        );
    }
